...
const HeaderScroll = () => {
  ...
  return (
    <div className="inner mt-3 px-4 position-relative">
      <div className="bg-red rounded-pill py-1 ">
        <nav className="d-flex justify-content-center">
          <ul className="nav flex-nowrap align-items-center">
            {menu.slice(0, 9).map((item, index) => (
              <li className="nav-item" key={index}>
                <Link
                  className="nav-link text-white uppercase title-nav"
                  to={item.path}
                >
                  {item.title}
                </Link>
            ...
            <li className="cursor-pointer" onClick={() => setShow(!show)}>
              {!show ? (
                <MdOutlineMoreHoriz size={28} color="white" />
              ) : (
